...
pub fn training_system(
    ...
    let (normalized_inputs, _original_inputs) = if data_model.is_normalized() {
        (dataset.inputs.clone(), dataset.inputs.clone())
    } else {
        normalize_data(&dataset.inputs)
    };

    let (batch_inputs, batch_outputs, test_inputs, test_outputs) = if train_ratio >= 0.999 {
        println!(
            "Using all {} samples for both training and evaluation (train_ratio=1.0)",
            n_samples
        );
        ...
        let test_outputs = select_elements(&dataset.outputs, &all_indices);

        (batch_inputs, batch_outputs, test_inputs, test_outputs)
    } else {
        let n_train = (n_samples as f64 * train_ratio) as usize;
        if n_train == 0 {
            training_state.error_message =
                Some("Training set is empty! Adjust train ratio.".to_string());
    ...
    if let Some(mut model) = training_state.selected_model.take() {
        ...
        match model.fit(&batch_inputs, &batch_outputs, learning_rate, 1) {
            Ok(train_loss) => {
                let fit_time = start_time.elapsed();
                match model.evaluate(&test_inputs, &test_outputs) {
                    Ok(test_loss) => {
                            ...
                            if let Ok(pred) = model.predict(&input) {
                                if model.is_classification() && pred.len() > 1 {
                                    ...
                                    println!(
                                        ...
                                        i,
                                        predicted_class,
                                        test_outputs[(i, 0)] as usize
                                    );
                                } else {
                                    println!(
                                        "  Sample {}: Prediction = {:.3}, Actual = {:.3}",
                                        i, pred[0], test_outputs[(i, 0)]
                                    );
                                }
                            }
                        }
                    ...
                    Err(e) => {
                        training_state.error_message =
                            Some(format!("Error evaluating model: {}", e));
            ...
            Err(e) => {
                training_state.error_message =
                    Some(format!("Error training model: {}", e));
...
fn select_rows(matrix: &DMatrix<f64>, indices: &[usize]) -> DMatrix<f64> {
    ...
    DMatrix::from_fn(indices.len(), matrix.ncols(), |i, j| matrix[(indices[i], j)])
}

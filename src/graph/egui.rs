...
pub fn draw_plot(mut contexts: EguiContexts, points: Res<Points>) {
        ...
        .show(ctx, |ui| {
            ...
            Plot::new("Data").view_aspect(2.0).show(ui, |plot_ui| {
                for (color, points) in hashmap_points {
                    plot_ui.points(
                        egui_plot::Points::new(PlotPoints::new(points))
                            .color(color)
                            .radius(2.0),
                    );
                }
...
fn bevy_color_to_egui(c: Color) -> Color32 {
    ...
    Color32::from_rgba_unmultiplied((r * 255.) as u8, (g * 255.) as u8, (b * 255.) as u8, 255)
}